    ...
    try:
        ...
        for match_id, home_team, away_team, season in matches:
            ...
            for i in range(len(snapshot_list) - 1):
                ...
                # Conservative: only bet now if odds won't improve by more than 0.5%
                threshold = 0.998  # 0.5% tolerance
                should_bet_home_now = current_home >= (best_future_home * threshold)
                should_bet_draw_now = current_draw >= (best_future_draw * threshold)
                should_bet_away_now = current_away >= (best_future_away * threshold)
