#!/usr/bin/env python3
"""
Step 6: Train Model 4 - Draw Betting Timing (Precision-Focused)
...
def load_prepared_data(data_dir='../../data/prepared'):
    ...
    print("=" * 60)
    print("STEP 6: Train Draw Betting Timing Model (PRECISION-FOCUSED)")
    print("=" * 60)
...
def train_regularized_models(X_train, y_train, X_val, y_val):
    ...
    print("\n💡 Based on Home Betting Timing model results and your precision priority:")
    print("   - Strong regularization worked best")
    print("   - Will add threshold calibration to increase precision")
    print("   - Goal: When model says 'Bet on Draw Now', it should be right!")

    models = {}
    thresholds = {}
    threshold_results = {}

    # Strategy 1: Strong regularization (what worked for Home model)
...
def save_model(model, threshold, model_name, output_dir='../../models'):
    ...
    model_path = f'{output_dir}/model_draw_timing_{model_name}_precision.pkl'
    threshold_path = f'{output_dir}/threshold_draw_timing_{model_name}_precision.txt'
...
def main():
    ...
    print(f"\n✓ Model 4: Precision-Focused Draw Betting Timing trained!")
    ...
    print("\n✨ You now know WHEN to bet on Draw - with high confidence!")
