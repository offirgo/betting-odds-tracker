#!/usr/bin/env python3
"""
Step 5: Train Model 3 - Home Betting Timing (Precision-Focused)
...
def load_prepared_data(data_dir='../../data/prepared'):
    ...
    print("=" * 60)
    print("STEP 5: Train Home Betting Timing Model (PRECISION-FOCUSED)")
    print("=" * 60)
...
def check_data_balance(y_train, y_val, y_test):
    ...
    print(f"\n📊 Validation Set:")
    print(f"   Bet Home NOW (1):     {val_counts.get(1, 0):,} ({val_yes_pct:.1f}%)")
...
def compare_all_models(X_train, X_val, X_test, y_train, y_val, y_test, models, thresholds):
    ...
    for r in results:
        ...
        precision_score = r['test_prec']  # Precision on test set
        capture_score = r['test_rec']  # Recall/capture rate
        overfit_score = max(0, 1 - r['overfitting_gap'] / 0.4)  # Overfitting control

        # NEW WEIGHTS: 60% precision, 20% capture, 20% overfit control
        r['combined_score'] = (0.6 * precision_score + 0.2 * capture_score + 0.2 * overfit_score)
...
def save_model(model, threshold, model_name, output_dir='../../models'):
    ...
    model_path = f'{output_dir}/model_home_timing_{model_name}_precision.pkl'
    threshold_path = f'{output_dir}/threshold_home_timing_{model_name}_precision.txt'
...
def main():
    ...
    print(f"\n✓ Model 3: Precision-Focused Home Betting Timing trained!")
    ...
    print("\n💡 Next steps:")
    print("   1. Train Model 4: Draw Betting Timing (Precision-Focused)")
    print("   2. Train Model 5: Away Betting Timing (Precision-Focused)")
    print("   3. Combine all models for complete arbitrage system!")

    print("\n✨ You now know WHEN to bet on Home team - with high confidence!")
    print("   When this model says 'bet now', it's much more likely to be correct.")
